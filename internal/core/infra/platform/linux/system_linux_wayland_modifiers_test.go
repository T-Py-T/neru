...
const testWlrootsCtrlModifier = "ctrl"
...
func TestWlrootsModifierDispatcherDeduplicatesNestedUsage(t *testing.T) {
	...
	err := dispatcher.event(testWlrootsCtrlModifier, true)
	if err != nil {
	...
	err = dispatcher.event(testWlrootsCtrlModifier, false)
	if err != nil {
	...
	want := []event{
		{modifier: testWlrootsCtrlModifier, isDown: true},
		{modifier: testWlrootsCtrlModifier, isDown: false},
	}
...
func TestWlrootsModifierDispatcherRetriesFailedFinalRelease(t *testing.T) {
	...
	dispatcher := newWlrootsModifierDispatcher(func(modifier string, isDown bool) error {
		if modifier != testWlrootsCtrlModifier {
